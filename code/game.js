...
function update() {	
	...
	// mouse is not over button (normal)
	if (hover == false) {
		ctx.fillStyle = "black";
		ctx.fillRect(button.x, button.y, button.width, button.height);
				ctx.fillStyle = "white";
		ctx.font = "30pt Calibri";
		ctx.fillText('START', button.x + 52, button.y + 35);
	}
	// player is hovering over the button (highlighted)
	else {
		ctx.fillStyle = "blue";
		ctx.fillRect(button.x, button.y, button.width, button.height);
				ctx.fillStyle = "white";
...
canvas.addEventListener('mousedown', function(evt) {
	...
	if (clickPos.x > button.x && clickPos.x < button.x+button.width && clickPos.y > button.y && clickPos.y < button.y+button.height) {
...
// get things started at page load
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    var getCoin = new Audio('Coin - Sound Effect.mp3');
    getCoin.play();
...
function runGame(plans, Display) {
  ...
  var audio = new Audio('FabrikC - N H N S [Noisuf-X Remix].mp3');
  audio.play();
